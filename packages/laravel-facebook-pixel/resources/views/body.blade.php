@if ($metaPixel->isEnabled())
    @unless (empty($eventLayer))
        <!-- Meta Pixel Events -->
        <script id="fb-pixel-standard-events" data-fb-pixel-tracking>
            (function() {
                // Initialize tracking store if not exists
                ...
                @foreach ($eventLayer as $eventName => $metaPixel)
                    (function() {
                        ...
                        // Skip if already tracked to prevent duplicates during SPA navigation
    ...
    @unless (empty($customEventLayer))
        ...
        <script id="fb-pixel-custom-events" data-fb-pixel-tracking>
            (function() {
                // Initialize tracking store if not exists
                if (!window.__fbPixelTrackedEvents) {
                    window.__fbPixelTrackedEvents = new Set();
