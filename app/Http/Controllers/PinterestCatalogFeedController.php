...
final class PinterestCatalogFeedController extends Controller
{
    public function __invoke(): StreamedResponse
    {
        return response()->stream(function (): void {
            ...
            echo "  <channel>\n";
            echo '    <title>'.$this->xml((config('app.name') ?: 'Store').' Pinterest Catalog Feed')."</title>\n";
            echo '    <link>'.$this->xml(url('/'))."</link>\n";
            echo '    <description>'.$this->xml('Live product data for Pinterest Catalogs')."</description>\n";
                        ...
                        foreach ($offers as $offer) {
                            try {
                                ...
                                if ($errors !== []) {
                                    ...
                                    Log::warning('Pinterest feed product skipped', [
                                        'product_id' => $offer->id,
                                        'parent_id' => $parent->id,
                                        'reasons' => $errors,
                                    ]);
                                ...
                                Log::error('Pinterest feed product failed', [
                                    'product_id' => $offer->id,
                                    'parent_id' => $parent->id,
            ...
            Log::info('Pinterest feed generated', [
                'included_products' => $included,
                'skipped_products' => $skipped,
            ]);
        }, 200, [
            'Content-Type' => 'application/xml; charset=UTF-8',
            'Content-Disposition' => 'inline; filename="pinterest-catalog-feed.xml"',
    ...
    private function googleProductCategory(
        ...
        /*
         * Pinterest-only exact product overrides.
         * Google Merchant feed does not use this config.
         */
        $overrides = config('pinterest_gpc', []);

        if (isset($overrides[(int) $product->id])) {
            return (string) $overrides[(int) $product->id];
        }
