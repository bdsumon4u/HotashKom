...
class ProductController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        return DataTables::eloquent($query)
            ->addIndexColumn()
            ->addColumn('image', fn(Product $product) => '<img src="'.asset(optional($product->base_image)->src).'" width="100" height="100" />')
            ->editColumn('name', fn(Product $product) => '<a href="'.route('products.show', $product).'" target="_blank">'.$product->name.'</a>')
            ->addColumn('pricing', fn(Product $product) => $product->price == $product->selling_price
            ...
            ->addColumn('stock', fn(Product $product) => $product->should_track
                ? '<span class="text-'.($product->stock_count ? 'success' : 'danger').'">'.$product->stock_count.' In Stock</span>'
                : '<span class="text-success">In Stock</span>')
            ->addColumn('actions', fn(Product $product) => '<div>
